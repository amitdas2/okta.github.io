...
3.  Configure CORS support in your Okta organization
4.  Serve the HTML file with a web server
...
You can enable Cross-Origin Resource Sharing (CORS) by following the steps in our [guide for Enabling CORS](/docs/api/getting_started/enabling_cors.html). Configure CORS using the same base URL as the web server you are using to host the HTML for the Okta Sign-In Widget (see below for instructions). For example, if you're serving the page using [Httpster](https://github.com/SimbCo/httpster), the origin would be `http://localhost:3333`).
...
If you are a Node.js developer, you might be more comfortable running a simple HTTP server in Node, which you can do as follows:
...
If now open the page with the Widget code on it, you can enter in the credentials for a user in your Okta org and you will be logged in. And that's it!
...
3.  Configured CORS support in your Okta organization
4.  Served the HTML file with a web server
...
Replace `http://localhost:8000/signed-in.html` in the `redirectUrl` variable with the URL of the landing page in your own web application where you would like to redirect the user after a successful login.
...
`OktaSignIn()` is below. If you'd like to see the full list of configuration options, see the [Sign-in Widget Reference page](https://developer.okta.com/code/javascript/okta_sign-in_widget_ref.html#configuration).
